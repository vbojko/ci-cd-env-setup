...
function install_pipeline()
{
    ...
    gitlab_archive='1540557175_2018_10_26_11.2.3_gitlab_backup.tar'
    ...
    docker rm jenkins
    docker rmi jenkins-python-ansible-aws
    if [[ "$unamestr" == 'Linux' ]]; then
       sh jenkins-ansible-aws/setup-jenkins.sh $PWD /usr/bin/docker
    elif [[ "$unamestr" == 'Darwin' ]]; then
       sh jenkins-ansible-aws/setup-jenkins.sh $PWD /usr/local/bin/docker
    fi
